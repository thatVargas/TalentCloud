    ...
    switch ($requestMethod) {
        ...
        case 'PUT':
            ...
            if(strlen($requestParams) > 1){
                ...
                $fileOwnerType = Utils::getParameterFromRequest($requestParams, 3);
                
                $fileUploadJSON = json_decode($jsonBody, TRUE);
                //var_dump($jobSeekerJSON);
                $user_id = Utils::getParameterFromRequest($requestParams,4);
                $fileUpload = new FileUpload();
                $fileUpload->setFile_id($fileUploadJSON["file_id"]);
                $fileUpload->setFile_name($fileUploadJSON["file_name"]);
                $fileUpload->setFile_content($fileUploadJSON["file_content"]);
                $fileUpload->setLast_updated($fileUploadJSON["last_updated"]);
                
                $result = FileUploadController::addFileUpload($fileUpload);
